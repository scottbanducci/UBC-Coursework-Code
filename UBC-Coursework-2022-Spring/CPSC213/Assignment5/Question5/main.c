...
struct testfunc {
  ...
  int heapsize;
  testfunc_t func;
  struct heap *heap; // filled in by test routine
};
...
int test_sanity(struct testfunc *tf) {
  ...
  void *ptr = mymalloc(tf->heap, SANITY_SIZE);
...
int test_freelist1(struct testfunc *tf) {
  ...
  void *ptr = mymalloc(tf->heap, FREELIST1_SIZE);
  ...
  void *ptr2 = mymalloc(tf->heap, FREELIST1_SIZE);
...
int test_freelist2(struct testfunc *tf) {
  ...
  for(int i=0; i<2; i++) {
    void *ptr = mymalloc(tf->heap, FREELIST2_SIZE);
...
int test_simple(struct testfunc *tf) {
  ...
  for(int i=0; i<SIMPLE_ITERS; i++) {
    ...
    if(ptrs[idx]) {
      myfree(tf->heap, ptrs[idx]);
      ptrs[idx] = NULL;
    } else {
      void *ptr = mymalloc(tf->heap, sizes[idx]);
...
int test_coalesce2(struct testfunc *tf) {
  ...
  */
  ...
  for(int i=0; i<COALESCE2_COUNT; i++) {
    myfree(tf->heap, ptrs[i]);
    ptrs[i] = NULL;
  }

  void *ptr = mymalloc(tf->heap, COALESCE2_BIGSIZE);
