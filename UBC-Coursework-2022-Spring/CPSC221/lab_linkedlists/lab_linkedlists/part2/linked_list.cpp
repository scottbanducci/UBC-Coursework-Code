...
Node* interleave(Node* list1, Node* list2){
  ...
  newList2 = list2;
    ...
    result = newList1;
    result->next = newList2;
    
    if (list1->next !=NULL && list2->next !=NULL){
      result->next->next = interleave(list1->next, list2->next);
    } else if (list1->next == NULL && list2->next != NULL){
      result->next->next = interleave(list2->next, list2->next);
    } else if (list1->next != NULL && list2->next == NULL){
      result->next->next = interleave(list1->next, list1->next);
    }
    return result;
}
